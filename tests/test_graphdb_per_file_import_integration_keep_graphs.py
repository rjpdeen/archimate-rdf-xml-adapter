...
def test_imports_two_fixture_xml_files_into_distinct_named_graphs_and_keeps_them() -> None:
    root = _project_root()
    xml_file_a, xml_file_b = _fixture_xml_files()[:2]

    test_run_id = uuid4().hex
    base_graph_iri = f"https://example.org/graph/test-import-{test_run_id}"
    ...
    print("Created named graphs:")
    print(graph_iri_a)
    print(graph_iri_b)
    print("Use the cleanup script with these IRIs or with the base prefix:")
    print(base_graph_iri)
